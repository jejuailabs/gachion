...
(() => {
  ...
  let transitionTimer;
  function apply(mode,announce=false){const readingY=window.scrollY;if(!worlds[mode])mode='light';root.dataset.mode=mode;buttons.forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.modeChoice===mode)));document.querySelector('meta[name="theme-color"]').content=worlds[mode].color;document.querySelector('#world-number').textContent=`0${Object.keys(worlds).indexOf(mode)+1} / 05`;document.querySelector('#world-caption').textContent=worlds[mode].caption;if(announce){try{localStorage.setItem('gachi-design-mode',mode);}catch{}document.querySelector('#mode-status').textContent=worlds[mode].name+' 모드로 변경했습니다.';clearTimeout(transitionTimer);root.classList.remove('world-switch');requestAnimationFrame(()=>{root.classList.add('world-switch');transitionTimer=setTimeout(()=>root.classList.remove('world-switch'),650);});}document.dispatchEvent(new Event('design-mode-change'));if(announce){window.scrollTo({top:readingY,behavior:'instant'});requestAnimationFrame(()=>window.scrollTo({top:readingY,behavior:'instant'}));}}
  buttons.forEach((b,i)=>{b.addEventListener('click',()=>{if(root.dataset.mode!==b.dataset.modeChoice)apply(b.dataset.modeChoice,true);});b.addEventListener('keydown',e=>{let next;if(e.key==='ArrowRight')next=(i+1)%5;if(e.key==='ArrowLeft')next=(i+4)%5;if(e.key==='Home')next=0;if(e.key==='End')next=4;if(next!==undefined){e.preventDefault();buttons[next].focus();buttons[next].click();}});});apply(root.dataset.mode);
  const TAU=Math.PI*2;
