...
(() => {
  ...
  Object.assign(worlds,{ayush:{name:'Ayush',caption:'상상한 것을, 눈앞의 현실로',color:'#dce5ff'},axle:{name:'Axle Journey',caption:'우리 동네에서 출발하는 변화의 여정',color:'#d4d9dc'},mindora:{name:'Mindora',caption:'작은 질문이 머물고, 가능성이 자라는 곳',color:'#f4f8f6'},frontier:{name:'Frontier',caption:'서로 다른 가능성이 함께 피어나다',color:'#050505'}});
  const buttons=[...document.querySelectorAll('[data-mode-choice]')];
  let transitionTimer;
  function apply(mode,announce=false){const readingY=window.scrollY;const anchor=announce&&readingY>700?[...document.querySelectorAll('main h2,main h3,.experiment-steps li,.work-card')].filter(e=>e.getBoundingClientRect().top>=145).sort((a,b)=>a.getBoundingClientRect().top-b.getBoundingClientRect().top)[0]:null;const anchorTop=anchor?.getBoundingClientRect().top;const restore=()=>window.scrollTo({top:anchor?window.scrollY+anchor.getBoundingClientRect().top-anchorTop:readingY,behavior:'instant'});if(!worlds[mode])mode='light';root.dataset.mode=mode;buttons.forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.modeChoice===mode)));document.querySelector('meta[name="theme-color"]').content=worlds[mode].color;document.querySelector('#world-number').textContent=`0${Object.keys(worlds).indexOf(mode)+1} / 09`;document.querySelector('#world-caption').textContent=worlds[mode].caption;if(announce){try{localStorage.setItem('gachi-design-mode',mode);}catch{}document.querySelector('#mode-status').textContent=worlds[mode].name+' 모드로 변경했습니다.';clearTimeout(transitionTimer);root.classList.remove('world-switch');requestAnimationFrame(()=>{root.classList.add('world-switch');transitionTimer=setTimeout(()=>root.classList.remove('world-switch'),650);});}document.dispatchEvent(new Event('design-mode-change'));if(announce){restore();requestAnimationFrame(restore);}}
  buttons.forEach((b,i)=>{b.addEventListener('click',()=>{if(root.dataset.mode!==b.dataset.modeChoice)apply(b.dataset.modeChoice,true);});b.addEventListener('keydown',e=>{let next;if(e.key==='ArrowRight')next=(i+1)%buttons.length;if(e.key==='ArrowLeft')next=(i+buttons.length-1)%buttons.length;if(e.key==='Home')next=0;if(e.key==='End')next=buttons.length-1;if(next!==undefined){e.preventDefault();buttons[next].focus();buttons[next].click();}});});apply(root.dataset.mode);
  const TAU=Math.PI*2;
