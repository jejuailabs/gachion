...
(() => {
  ...
  function draw(){if(!ctx||!width)return;ctx.clearRect(0,0,width,height);ctx.globalCompositeOperation='lighter';
  ...
  addEventListener('scroll',()=>{if(!scrollFrame)scrollFrame=requestAnimationFrame(scroll);},{passive:true});addEventListener('resize',scroll,{passive:true});scroll();resize();state();
})();
